...
var CommonData = (function () {
    function CommonData() {
        this.noBrainSurface = false;
        this.nodeIDUnderPointer = [-1, -1, -1, -1, -1];
    ...
    CommonData.prototype.notifyCoords = function () {
        this.coordCallbacks.forEach(function (c) {
            c();
        });
    };
    CommonData.prototype.notifyLabels = function () {
        this.labelCallbacks.forEach(function (c) {
            c();
        });
    };
    CommonData.prototype.notifySurface = function () {
        this.surfaceCallbacks.forEach(function (c) {
            c();
        });
    };
    return CommonData;
...
var DataSet = (function () {
    function DataSet() {
        this.simCallbacks = new Array();
        this.attCallbacks = new Array();
    }
    ...
    DataSet.prototype.notifySim = function () {
        this.simCallbacks.forEach(function (c) {
            c();
        });
    };
    DataSet.prototype.notifyAttributes = function () {
        this.attCallbacks.forEach(function (c) {
            c();
        });
    };
    return DataSet;
...
var Attributes = (function () {
    function Attributes(text) {
        ...
        var values = new Array(numAttributes);
        for (var i = 0; i < numAttributes; ++i) {
            this.columnNames[i].trim();
            values[i] = new Array(this.numRecords);
        }
        for (var i = 1; i <= this.numRecords; ++i) {
    ...
    Attributes.prototype.getRecord = function (index) {
        var record = '';
...
var DummyApp = (function () {
    ...
    DummyApp.prototype.setDataSet = function () {
    };
    DummyApp.prototype.resize = function () {
    };
    DummyApp.prototype.applyFilter = function () {
    };
    DummyApp.prototype.setNodeDefaultSizeColor = function () {
    };
    DummyApp.prototype.setNodeSize = function () {
    };
    DummyApp.prototype.setNodeColor = function () {
    };
    DummyApp.prototype.setNodeColorDiscrete = function () {
    };
    DummyApp.prototype.setANodeColor = function () {
    };
    DummyApp.prototype.setEdgeSize = function () {
    };
    DummyApp.prototype.setCircularBarColor = function () {
    };
    DummyApp.prototype.highlightSelectedNodes = function () {
    };
    DummyApp.prototype.isDeleted = function () {
    };
    DummyApp.prototype.save = function () {
    };

    //init() { }
    DummyApp.prototype.initEdgeCountSlider = function () {
    };
    DummyApp.prototype.initShowNetwork = function () {
    };
    return DummyApp;
})();
...
function loadExampleData() {
    $.get('data/coords.txt', function (text) {
        ...
        //$('#shared-coords').css({ color: 'green' });
        $('#label-coords').text("default data").css({ color: 'green' });
    });
    $.get('data/mat1.txt', function (text) {
        parseSimilarityMatrix(text, dataSets[0]);

        //$('#d1-mat').css({ color: 'green' });
        $('#label-similarity-matrix').text("default data").css({ color: 'green' });
    });
    $.get('data/attributes1.txt', function (text) {
        parseAttributes(text, dataSets[0]);

        //$('#d1-att').css({ color: 'green' });
        $('#label-attributes').text("default data").css({ color: 'green' });
    ...
    $.get('data/labels.txt', function (text) {
        ...
        //$('#shared-labels').css({ color: 'green' });
        $('#label-labels').text("default data").css({ color: 'green' });
    });
...
function setupAttributeTab() {
    if (dataSets[0].attributes) {
        ...
        if ($('#div-node-size').length > 0)
            divNodeSizeRange = $('#div-node-size').detach();
        if ($('#div-node-color-pickers').length > 0)
            divNodeColorPickers = $('#div-node-color-pickers').detach();
        if ($('#div-node-color-pickers-discrete').length > 0)
            divNodeColorPickersDiscrete = $('#div-node-color-pickers-discrete').detach();
...
function applyFilterButtonOnClick() {
    ...
    if (apps[0])
        apps[0].applyFilter(idArray);
    if (apps[1])
        apps[1].applyFilter(idArray);
    if (apps[2])
        apps[2].applyFilter(idArray);
    if (apps[3])
        apps[3].applyFilter(idArray);
...
function setSelectNodeKeyBackgroundColor(color) {
...
function setNodeSizeOrColor() {
    ...
    if (sizeOrColor == "node-size") {
        ...
        var newScaleArray = scaleArray.map(function (value) {
            return scaleMap(value);
        });

        if (apps[0])
            apps[0].setNodeSize(newScaleArray);
        if (apps[1])
            apps[1].setNodeSize(newScaleArray);
        if (apps[2])
            apps[2].setNodeSize(newScaleArray);
        if (apps[3])
            apps[3].setNodeSize(newScaleArray);
        ...
        if (attribute == "module_id") {
            ...
            if (apps[0])
                apps[0].setNodeColorDiscrete(attribute, keyArray, colorArray);
            if (apps[1])
                apps[1].setNodeColorDiscrete(attribute, keyArray, colorArray);
            if (apps[2])
                apps[2].setNodeColorDiscrete(attribute, keyArray, colorArray);
            if (apps[3])
                apps[3].setNodeColorDiscrete(attribute, keyArray, colorArray);
            ...
            if (apps[0])
                apps[0].setNodeColor(attribute, minColor, maxColor);
            if (apps[1])
                apps[1].setNodeColor(attribute, minColor, maxColor);
            if (apps[2])
                apps[2].setNodeColor(attribute, minColor, maxColor);
            if (apps[3])
                apps[3].setNodeColor(attribute, minColor, maxColor);
        ...
        if (apps[0])
            apps[0].setNodeDefaultSizeColor();
        if (apps[1])
            apps[1].setNodeDefaultSizeColor();
        if (apps[2])
            apps[2].setNodeDefaultSizeColor();
        if (apps[3])
            apps[3].setNodeDefaultSizeColor();
    }
...
$('#select-node-key').on('change', function () {
    ...
    for (var i = 0; i < keySelection.length; i++) {
        if (keySelection.options[i].value == key) {
            var color = keySelection.options[i].style.backgroundColor;
            var hex = colorToHex(color);
            document.getElementById('input-node-color').color.fromString(hex.substring(1));
            break;
        }
    }
});
...
function getNodeScaleArray(attribute) {
    ...
    var scaleFactor = 0.5;
    if (max / min > 10) {
        scaleArray = attrArray.map(function (value) {
            return scaleFactor * Math.log(value) / Math.log(min);
        });
    } else {
        scaleArray = attrArray.map(function (value) {
            return scaleFactor * value / min;
        });
    }
...
function setupNodeSizeRangeSlider(attribute) {
    if ($('#div-node-color-pickers').length > 0)
        divNodeColorPickers = $('#div-node-color-pickers').detach();
    if ($('#div-node-color-pickers-discrete').length > 0)
        divNodeColorPickersDiscrete = $('#div-node-color-pickers-discrete').detach();
...
function setupColorPickerDiscrete(attribute) {
    if ($('#div-node-size').length > 0)
        divNodeSizeRange = $('#div-node-size').detach();
    if ($('#div-node-color-pickers').length > 0)
        divNodeColorPickers = $('#div-node-color-pickers').detach();
    ...
    var uniqueColors = uniqueKeys.map(function (group) {
        return d3ColorSelector(group);
    });
    ...
    document.getElementById('input-node-color').color.fromString(uniqueColors[0].substring(1));
}
...
var brainSurfaceColor = "0xe3e3e3";
...
function initNodeColor() {
    if ((loadObj.nodeColorAttribute != null) && (loadObj.nodeColorAttribute.length > 0)) {
        ...
        if (loadObj.nodeColorAttribute == "module_id") {
            ...
            document.getElementById('input-node-color').color.fromString(loadObj.nodeColorDiscrete[0].substring(1));

            setNodeSizeOrColor();
        } else {
            document.getElementById('input-min-color').color.fromString(loadObj.nodeColorContinuousMin.substring(1));
            document.getElementById('input-max-color').color.fromString(loadObj.nodeColorContinuousMax.substring(1));
...
function loadBrainModel(fileName) {
...
function loadAttributes(file, dataSet) {
    var reader = new FileReader();
    reader.onload = function () {
...
function parseAttributes(text, dataSet) {
...
function setupCrossFilter(attrs) {
    ...
    dc.dataCount(".dc-data-count").dimension(cfilter).group(all);
    ...
    for (var j = 0; j < attrs.columnNames.length; ++j) {
        ...
        var dim = cfilter.dimension(function (d) {
            return d[columnName];
        });
        dimArray.push(dim);
